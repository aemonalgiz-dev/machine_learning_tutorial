...
// coefficients by name and the R-squared. At degree 2 on the worked example
// the physics comes back out of the data. At high degree the curve chases
// every point and swings wildly between them, which is the page's warning.
// Every fit is the library's through the API, not the browser's.
...
function statusText(fit: PolynomialFit | null, degree: number): string {
  if (!fit) return "…";
  if (degree === 1) {
...
export function PolynomialPlayground() {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      try {
        setFit(await fitPolynomial(points, degree));
        setMessage(null);
      } catch (error) {
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {fit?.coefficients.map((coefficient) => (
          <Chip
            ...
            value={formatCoefficient(coefficient.value)}
          />
        ))}
      </div>

      <p className="mt-3 text-sm text-slate-600 dark:text-slate-400">
        {message ? message : statusText(fit, degree)}
      </p>
    </div>
  );
}
