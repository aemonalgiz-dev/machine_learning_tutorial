...
// and the colours you see are the groups the fit found, with an X at each
...
import { ClusterFit, fitClusters } from "@/lib/concepts/k-means";
import { CROWD, IDEAL_CASE, WORKED_PEOPLE, groupColour, randomClumps } from "./kMeansFixtures";
...
function toPixel(point: Point) {
  ...
  const py = PAD.top + (1 - (point.y - DOMAIN.yMin) / (DOMAIN.yMax - DOMAIN.yMin)) * PLOT.height;
  return { px, py };
}
...
function toData(px: number, py: number): Point {
  ...
  return {
    x: clamp(DOMAIN.xMin + ((px - PAD.left) / PLOT.width) * (DOMAIN.xMax - DOMAIN.xMin), DOMAIN.xMin, DOMAIN.xMax),
    y: clamp(DOMAIN.yMin + (1 - (py - PAD.top) / PLOT.height) * (DOMAIN.yMax - DOMAIN.yMin), DOMAIN.yMin, DOMAIN.yMax),
  };
}

const BUTTON = "rounded-md border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700";

export function KMeansPlayground() {
  const [points, setPoints] = useState<Point[]>(WORKED_PEOPLE);
  const [k, setK] = useState(2);
  const [answer, setAnswer] = useState<ClusterFit | null>(null);
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      try {
        setAnswer(await fitClusters(points, k));
        setMessage(null);
      } catch (error) {
  ...
  const onPointerMove = (event: React.PointerEvent) => {
    ...
    setPoints((current) => current.map((point, i) => (i === index ? moved : point)));
  };
  ...
  const regionPixel = (column: number, row: number) => {
    ...
    const x = regions.x_min + ((column + 0.5) / regions.cells) * (regions.x_max - regions.x_min);
    const y = regions.y_min + ((row + 0.5) / regions.cells) * (regions.y_max - regions.y_min);
    return toPixel({ x, y });
  };
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2 pb-3">
        <button onClick={() => setPoints(IDEAL_CASE)} className={BUTTON}>
          An Ideal Case
        </button>
        <button onClick={() => setPoints(WORKED_PEOPLE)} className={BUTTON}>
          Two small clumps
        </button>
        <button onClick={() => setPoints(CROWD)} className={BUTTON}>
          The crowd, unlabelled
        </button>
        <button onClick={() => setPoints(randomClumps())} className={BUTTON}>
          Random clumps
        </button>
        <label className="ml-auto flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
          k
          <input type="range" min={1} max={6} step={1} value={k} onChange={(event) => setK(Number(event.target.value))} className="w-32 accent-indigo-600" />
        ...
        {points.map((point, index) => {
          ...
          return (
            <circle
              ...
              cy={py}
              r={6}
              fill={label === undefined ? "#64748b" : groupColour(label)}
              className="cursor-grab stroke-white dark:stroke-slate-900"
              strokeWidth={1.5}
        ...
        {answer?.centres.map((centre, index) => {
          ...
          return (
            <g key={`c${index}`} className="text-slate-700 dark:text-slate-200">
              <line x1={px - 7} y1={py - 7} x2={px + 7} y2={py + 7} stroke="currentColor" strokeWidth={3} />
              <line x1={px - 7} y1={py + 7} x2={px + 7} y2={py - 7} stroke="currentColor" strokeWidth={3} />
            </g>
          );
        })}
      ...
      <p className="mt-2 text-center text-xs text-slate-500 dark:text-slate-500">
        The colours are the groups the fit found, and each X is a group&rsquo;s resting centre. No dot carried a label to begin with.
      </p>

      <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat label="Groups asked for" value={String(k)} />
        <Stat label="Inertia" value={answer ? answer.inertia.toFixed(1) : "…"} />
        <Stat label="Passes to rest" value={answer ? String(answer.iterations_run) : "…"} />
        <Stat label="Group sizes" value={answer ? answer.sizes.join(", ") : "…"} />
      </div>
