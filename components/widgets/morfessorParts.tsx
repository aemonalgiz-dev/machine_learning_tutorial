...
// The pieces every widget on the Morfessor page draws.
//
...
// full contrast, and the stand-in a table produces when it cannot spell
// something, which is the one piece a reader should notice. Both live here so
// the six widgets draw them the same way, and so a change to the marker is one
// edit. The API computes; these only draw.
...
// One piece as a chip. `tone` colours the whole chip where a widget is
// contrasting two tables; the stand-in overrides it, since a reader should
...
// Code lengths run to four decimals across the page, so one place decides it.
export function nats(value: number): string {
  return value.toFixed(4);
}

export const CORPUS_LABELS: Record<string, string> = {
  inflections: "Twelve inflected forms",
  sentences: "Eighteen sentences",
  prefixes: "Two prefixes on two roots",
};
